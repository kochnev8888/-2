async function getCurrentWeatherByCity(city) {
    const url = `http://api.weatherapi.com/v1/current.json?key=9fc1ec51d26f4189af2220001240112&q=${city}&aqi=no`;
...
async function getForecastByCity(city) {
    const url = `http://api.weatherapi.com/v1/forecast.json?key=9fc1ec51d26f4189af2220001240112&q=${city}&days=1&aqi=no&alerts=no`;
    const data = await fetch(url);
...
function createForecastElement(iconSrc, time, temperature) {
    ...
    forecastElement.setAttribute('class',"current-element")
    ...
    const forecastTemperature = document.createElement('img')
